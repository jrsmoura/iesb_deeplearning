...
%============================================================
\documentclass[8pt, aspectratio=169,10pt]{beamer}
...
\end{block}
\column{0.48\textwidth}
\begin{block}{Hinge Loss (SVM)}
\[
\mathcal{L}_{\text{hinge}} = \frac{1}{N}\sum_i \max(0,\, 1-y_i\hat{y}_i)
\]
\small $y_i \in \{-1,+1\}$. Não probabilístico; usado em SVMs.
\end{block}

\begin{block}{Focal Loss (para desbalanceamento)}
\[
\mathcal{L}_{\text{focal}} = -\frac{1}{N}\sum_i (1-\hat{y}_{i,c_i})^\gamma \log\hat{y}_{i,c_i}
\]
\small $\gamma>0$ reduz peso de exemplos fáceis (Lin et al., 2017).
\end{block}
...
\small
$\odot$ denota produto elemento a elemento (Hadamard). A multiplicação por $f'(\bz^{(\ell)})$ é a \textbf{gate} do gradiente: ativações saturadas ``bloqueiam'' o gradiente.
\end{exampleblock}
